...
const input =
  'w-full rounded-[6px] border border-[var(--border)] bg-[var(--background)] px-2.5 py-1.5 text-[12px] focus:border-[var(--border-strong)] disabled:opacity-50';
const button =
...
export function AddSiteForm({ canEdit }: { canEdit: boolean }) {
...
export function EditSiteForm({ site, canEdit }: { site: SiteRef; canEdit: boolean }) {
...
export function PsiKeyForm({ site, canEdit }: { site: SiteRef; canEdit: boolean }) {
    ...
    <form action={action} className="space-y-3">
      ...
      <fieldset disabled={!canEdit} className="space-y-3">
        ...
        >
          <PasswordInput
            name="psiApiKey"
            // The real key is never sent to the browser. An untouched field
            // keeps the stored value rather than overwriting it with these dots.

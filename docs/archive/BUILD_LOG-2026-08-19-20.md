# Build Log Archive — 19–20 Aug 2026

> Split out of `docs/BUILD_LOG.md` during a docs-cleanup pass (22 Aug 2026) —
> that file had grown to 2,400+ lines, most of it early build history that a
> fresh agent no longer needs to read to understand the app's current state.
> Nothing was deleted; this is the same content, moved.
>
> **This file is historical, not authoritative.** The "What this project is,"
> milestone table, and "decisions settled" sections below reflect
> understanding AS OF 20 Aug 2026 — several are stale now (e.g. "BullMQ +
> Redis," "worker concurrency 20") and were superseded by later work recorded
> in the live `docs/BUILD_LOG.md`. For current facts, use `CLAUDE.md`,
> `docs/DECISIONS.md`, and `docs/IMPLEMENTATION_PLAN.md` — never this file.
>
> Covers: M0 through M10 (stage 1-2 build), stages 3-6, the multi-tenant
> rebuild, the interface rebuild, the Vercel/Workflow migration, and bug
> fixes through "the scheduler was never actually being ticked." The Phase 5
> per-tenant cutover and onboarding tour work (21-22 Aug 2026 onward) is
> still in the live `docs/BUILD_LOG.md`.
